...
module.exports = class userinfo extends Command {
  ...
  async run(message, args) {
    ...
    if (Jeth) {
      ...
      const userFlags = pUser.user.flags || await pUser.user.fetchFlags();
      if (userFlags.has('64')) flags.push('<:Bravery:938280523153965097>');
      if (userFlags.has('128')) flags.push('<:Brilliance:938280523426578533>');
      if (userFlags.has('256')) flags.push('<:Balance:938280523409817600>');
    ...
    const normalUser = new MessageEmbed()
      ...
      .addField(`Meus Cargos`, pUser.roles.cache.map(role => role.toString()).join(' ').replace('@everyone', ' '), false)
